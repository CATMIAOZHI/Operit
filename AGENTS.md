...
- 新增用户可见文字使用资源字符串并提供简体中文；`values/`（默认简体中文）和 `values-en/`（英语）为唯一需要维护的文案来源，不新增或同步其他语言。`MissingTranslation` 检查已在 `personal/main` 的 `app/build.gradle.kts` 中禁用，其他语言的现有翻译由社区自行维护，不作为本仓库门槛。文档只在行为、接口、构建或使用方式变化时更新。
