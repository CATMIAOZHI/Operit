...
fun SpeechServicesSettingsScreen(
    ...
    if (currentTtsProfile == null || currentSttProfile == null ||
        ttsProfiles.none { it.id == currentTtsProfile?.id } ||
        sttProfiles.none { it.id == currentSttProfile?.id }) {
    ...
    var ttsServiceTypeInput by remember(currentTtsProfileId, ttsServiceType) { mutableStateOf(ttsServiceType) }
    var ttsUrlTemplateInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.urlTemplate) }
    var ttsApiKeyInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.apiKey) }
    var ttsHeadersInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(Json.encodeToString(httpConfig.headers)) }
    var ttsHttpMethodInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.httpMethod) }
    var ttsRequestBodyInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.requestBody) }
    var ttsContentTypeInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.contentType) }
    var ttsLocaleTagInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.localeTag) }
    var ttsVoiceIdInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.voiceId) }
    var ttsModelNameInput by remember(currentTtsProfileId, httpConfig) { mutableStateOf(httpConfig.modelName) }
    var ttsResponsePipelineInput by remember(currentTtsProfileId, httpConfig) {
        mutableStateOf(HttpTtsResponsePipelineStep.encodeList(httpConfig.responsePipeline))
    }
    var vitsPackagePathInput by remember(currentTtsProfileId, vitsConfig) { mutableStateOf(vitsConfig.packagePath) }
    var vitsSpeakerIdInput by remember(currentTtsProfileId, vitsConfig) { mutableStateOf(vitsConfig.speakerId) }
    var vitsOptionsInput by remember(currentTtsProfileId, vitsConfig) { mutableStateOf(Json.encodeToString(vitsConfig.options)) }
    var ttsSpeechRateInput by remember(currentTtsProfileId, ttsSpeechRate) { mutableStateOf(ttsSpeechRate) }
    var ttsPitchInput by remember(currentTtsProfileId, ttsPitch) { mutableStateOf(ttsPitch) }
    var ttsHeadersJsonError by remember(currentTtsProfileId) { mutableStateOf<String?>(null) }
    var ttsResponsePipelineJsonError by remember(currentTtsProfileId) { mutableStateOf<String?>(null) }
    var vitsOptionsJsonError by remember(currentTtsProfileId) { mutableStateOf<String?>(null) }
    var httpMethodDropdownExpanded by remember(currentTtsProfileId) { mutableStateOf(false) }
    val ttsCleanerRegexsState = remember(currentTtsProfileId) { mutableStateListOf(*ttsCleanerRegexs.toTypedArray()) }
    ...
    var simpleTtsVoices by remember(currentTtsProfileId) { mutableStateOf<List<VoiceService.Voice>>(emptyList()) }
    var simpleTtsVoicesLoading by remember(currentTtsProfileId) { mutableStateOf(false) }
    var simpleTtsVoicesError by remember(currentTtsProfileId) { mutableStateOf<String?>(null) }
    var simpleTtsLocaleExpanded by remember(currentTtsProfileId) { mutableStateOf(false) }
    var simpleTtsShowVoiceDialog by remember(currentTtsProfileId) { mutableStateOf(false) }

    // --- State for STT Settings ---
    val sttServiceType = activeSttProfile.serviceType
    val sttHttpConfig = activeSttProfile.httpConfig
    var sttServiceTypeInput by remember(currentSttProfileId, sttServiceType) { mutableStateOf(sttServiceType) }

    var sttEndpointUrlInput by remember(currentSttProfileId, sttHttpConfig) { mutableStateOf(sttHttpConfig.endpointUrl) }
    var sttApiKeyInput by remember(currentSttProfileId, sttHttpConfig) { mutableStateOf(sttHttpConfig.apiKey) }
    var sttModelNameInput by remember(currentSttProfileId, sttHttpConfig) { mutableStateOf(sttHttpConfig.modelName) }

    // 同步 DataStore 的数据到 State
    LaunchedEffect(currentTtsProfileId, ttsCleanerRegexs) {
