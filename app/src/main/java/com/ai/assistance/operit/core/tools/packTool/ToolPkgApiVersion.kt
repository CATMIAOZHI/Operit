...
private data class OperitVersion(
    ...
    companion object {
        /**
         * Only the leading `major.minor.patch` and the optional `+build` counter participate in
         * comparisons. Every other suffix (`-ry.N`, `-dev`, and any future release channel marker)
         * is ignored on purpose: failing to parse the host version would disable ToolPkg loading
         * and JS execution entirely, so unknown suffixes must degrade to "no build number" instead
         * of throwing.
         */
