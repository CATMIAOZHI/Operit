...
internal class ToolPkgHookExecutionBudget private constructor(
    ...
    companion object {
        /** Emitted by the JS engine and JS runtime when a script exceeds its deadline. */
        private const val TIMEOUT_MARKER = "Script execution timed out after"
    ...
    fun logTimeoutIfPresent(
        ...
        // Match only the engine's own timeout text. A plugin-authored failure that merely
        // mentions "timed out" must not end the chain early or claim the plugin was skipped.
        if (failureMessage?.contains(TIMEOUT_MARKER, ignoreCase = true) != true) {
            return false
        }
        AppLogger.w(
            tag,
