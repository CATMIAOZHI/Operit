 ...
 * submit_summary / submit_comments 只把候选写入本会话（不直接发布）。
 */
class ReadingCompanionRunSession(
    ...
    /** Summary-only manual run: submit_summary is the terminal operation and no comments are staged. */
    val summaryOnly: Boolean = false,
) {
    /** 本次 run 已接受的段评候选；发布由协调器走提交底线。 */
    @Volatile
    ...
    /**
     * 摘要与段评是否已提交并完成本 run（submit_comments 成功后置位）。
     * 与 stoppedReason 分开：coordinator 会把 stoppedReason 误判为 loop 异常，
     * 而 finalized 只拒绝后续工具调用，不改变 run 终态语义。
     */
    @Volatile
    var submissionFinalized: Boolean = false
        private set

    @Synchronized
    fun updateCandidateSummary(summary: String): Boolean {
        if (stoppedReason != null || submissionFinalized) return false
        candidateSummary = summary
        return true
    }

    @Synchronized
    fun tryFinalizeComments(drafts: List<AutoCommentDraft>): SubmitCommentsStatus {
        if (stoppedReason != null || submissionFinalized) {
            return SubmitCommentsStatus.ALREADY_FINALIZED
        }
        if (candidateSummary.isBlank()) {
            return SubmitCommentsStatus.SUMMARY_REQUIRED
        }
        candidateDrafts = drafts
        submissionFinalized = true
        return SubmitCommentsStatus.FINALIZED
    }

    @Synchronized
    fun tryFinalizeSummaryOnly(): SubmitCommentsStatus {
        if (stoppedReason != null || submissionFinalized) {
            return SubmitCommentsStatus.ALREADY_FINALIZED
        }
        if (candidateSummary.isBlank()) {
    ...
    fun targetChapter(): ReaderChapter =
        chapters.first { it.index == chapterIndex }

    fun chapterByRef(chapterRef: String): ReaderChapter? =
        chapters.firstOrNull {
            ReadingCompanionFileStore.chapterRef(bookId, it.sourceId) == chapterRef
        }
}

enum class SubmitCommentsStatus {
    FINALIZED,
    SUMMARY_REQUIRED,
    ALREADY_FINALIZED,
}
