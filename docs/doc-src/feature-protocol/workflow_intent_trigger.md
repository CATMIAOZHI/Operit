...
- `auth_token` 是执行工作流的能力凭据，由当前安装的私有密钥签名并使用 URL-safe 字符编码；密钥保存在不参与备份/设备迁移的私有目录。恢复工作流备份或密钥变化后，应用会在第一次读取内部定义时先轮换失效 token，再把新值显示给编辑器。必须复制编辑器生成的完整值，不要手工构造。不要放入公共仓库、截图、日志或回传 extras；怀疑泄露时，在编辑器中删除旧值并保存，让应用生成新的签名 token，再同步更新发送方。
